# 👥 Comptes Utilisateurs - Santé Rurale Mali

## Comptes disponibles pour connexion

### 👑 Administrateur
- **Email** : `admin@cscom-koulikoro.ml`
- **Mot de passe** : `Admin2024!`
- **Rôle** : Administrateur système
- **Nom** : Mamadou DIARRA
- **Permissions** : Accès complet à tous les modules

---

### 👨‍⚕️ Médecin
- **Email** : `dr.traore@cscom-koulikoro.ml`
- **Mot de passe** : `Medecin2024!`
- **Rôle** : Médecin
- **Nom** : Dr. Amadou TRAORÉ
- **Permissions** : Consultations, prescriptions, rapports

---

### 👩‍⚕️ Major
- **Email** : `major.kone@cscom-koulikoro.ml`
- **Mot de passe** : `Major2024!`
- **Rôle** : Major (Infirmier en chef)
- **Nom** : Fatimata KONÉ
- **Permissions** : Consultations, gestion des soins

---

### 🩺 Soignant
- **Email** : `soignant.coulibaly@cscom-koulikoro.ml`
- **Mot de passe** : `Soignant2024!`
- **Rôle** : Soignant
- **Nom** : Ibrahim COULIBALY
- **Permissions** : Enregistrement patients, consultations de base

---

## 📝 Notes

- Tous les comptes sont actifs et fonctionnels
- Les mots de passe respectent la politique de sécurité
- En production, ces comptes seront gérés par l'API backend
- Actuellement, l'authentification utilise un système mock pour le développement

---

## 🔒 Sécurité

Les mots de passe doivent respecter les critères suivants :
- Minimum 8 caractères
- Au moins une majuscule
- Au moins un chiffre
- Au moins un caractère spécial

---

## 🆘 Support

En cas de problème de connexion, contactez l'administrateur système.
